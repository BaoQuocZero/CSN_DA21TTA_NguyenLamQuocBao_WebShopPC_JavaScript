...
class Sliders extends React.Component {
    ...
    showSlides = (n) => {
        ...
        let slides = this.slidesRef.current ? this.slidesRef.current.getElementsByClassName("mySlides") : [];
        let dots = this.dotsRef.current ? this.dotsRef.current.getElementsByClassName("dot") : [];
        ...
        return (
            <>
                ...
                <div style={{ textAlign: "center" }} ref={this.dotsRef}>
                    {data &&
                        data.slice(0, 5).map((item, index) => (
                            <span key={item.MaSP} className="dot" onClick={() => this.currentSlide(index + 1)}></span>
                        ))
                    }
                </div>
            </>
